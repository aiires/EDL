...
function love.load()
	...
	-- Explicação: "vida" é uma variável Global, assim 
	-- o Binding time é feito em tempo de compilação.
	dic = {["l"] = larguraTela / 2, ["a"] = alturaTela / 2 + 10}
	-- TAREFA 6
	-- Tipo não primitivo: Dicionário
	...
	-- TAREFA 5
	-- Nome: Variável “button_play”
	-- Propriedade: tamanho da lista
	-- Binding time: compilação
	-- Explicação: "button_play" é uma lista que possui 4 variáveis com tamanhos definidos estaticamente 
	-- e que definirão o tamaho de "button_play" em tempo de compilação.
	...
	balde = {
		...
		pos = {X = dic.l,Y = dic.a};
		-- TAREFA 6
		-- Tipo não primitivo: Tupla
		ang = 0;
		vel = 300
	}
	-- TAREFA 6
	-- Tipo não primitivo: Registro
	...
	-- Explicação: "gotas" é uma lista que é inicializada vazia e que será alterada em
	-- tempo de execução pela função gota().

	-- TAREFA 6
	-- Tipo não primitivo: Array
end
...
function love.draw()
	...
	-- TAREFA 5
	-- Nome: estrutura do “if then”
	-- Propriedade: sintaxe
	-- Binding time: design
	-- Explicação: Como o "if then" é uma estrutura definida
	-- pela linguagem, o Binding time é feito na hora do Design.
		love.graphics.draw(fundo,planodefundo.x, planodefundo.y)
		love.graphics.draw(fundo2,planodefundo.x2, planodefundo.y)

		love.graphics.draw(balde.imagem,balde.pos.X,balde.pos.Y,ang,1,1,imgBalde:getWidth()/2,imgBalde:getHeight()/2)
		...
		-- Binding time: design
		-- Explicação: Como o ".." é operador, que tem a função de concatenar, é definido
		-- pela linguagem, o Binding time é feito na hora do Design.
	else
...
function movimento(dt)
	if love.keyboard.isDown("left") and balde.pos.X > imgBalde:getWidth()/2 then
		balde.pos.X = balde.pos.X - balde.vel * dt
		ang = -0.1
	elseif love.keyboard.isDown("right") and balde.pos.X < larguraTela - imgBalde:getWidth()/2 then
		balde.pos.X = balde.pos.X + balde.vel * dt
		ang = 0.1
	else
		ang = 0
	...
	if love.keyboard.isDown("down") and balde.pos.Y < alturaTela - imgBalde:getHeight()/2 then
		balde.pos.Y = balde.pos.Y + balde.vel * dt
	end


	if love.keyboard.isDown("up") and balde.pos.Y > alturaTela/2 then
		balde.pos.Y = balde.pos.Y - balde.vel * dt
	end
end
...
function gota(dt)
	...
	if tempoNovaGota < 0 then
		...
		-- TAREFA 5
		-- Nome: Variável “novaGota”
		-- Propriedade: endereço
		-- Binding time: execução
		-- Explicação: "novaGota" é uma variável local e por isso seu endereço é 
...
function catch()
	...
	for i, gota in ipairs(gotas) do
		if colisao(balde.pos.X,balde.pos.Y,balde.imagem:getWidth(),balde.imagem:getHeight(),gota.x,gota.y,gota.img:getWidth(),gota.img:getHeight()) then
			table.remove(gotas,i)
