...
notasaluno: Aluno -> List Float
notasaluno (_,n1,n2) = [n1,n2]
...
total: Float
total = List.foldl (\a b -> (media a) + b ) 0 turma

-- d) LISTA DE ALUNOS QUE GABARITARAM A P1 ([("Maria",10,8), ...])
turma_dez_p1: Turma
turma_dez_p1 = List.filter (\(_,n1,_) -> n1 ==10) turma

-- e) LISTA COM OS NOMES E MEDIAS DOS ALUNOS APROVADOS ([("Maria",9), ...])
aprovados2: List (String,Float)
aprovados2 = List.map  (\a -> (nome a,media a)) (List.filter (\a -> (media a) >= 7) turma)

-- f) LISTA COM TODAS AS NOTAS DE TODAS AS PROVAS ([7,4,10,8,...])
notas: List Float
notas = List.foldl (\a b -> List.append b (notasaluno a)) [] turma 
